...
abstract class SMTConverter extends (Formula => String) {
  ...
  private lazy val SMT_INTERPRETED_FUNCTIONS = Map[NamedSymbol, String](
    InterpretedSymbols.absF ->
      ("(define-fun " + nameIdentifier(InterpretedSymbols.absF) + " ((x Real)) Real\n  (ite (>= x 0) x (- x)))"),
    InterpretedSymbols.minF ->
      ("(define-fun " + nameIdentifier(InterpretedSymbols.minF) +
        " ((x1 Real) (x2 Real)) Real\n  (ite (<= x1 x2) x1 x2))"),
    InterpretedSymbols.maxF ->
      ("(define-fun " + nameIdentifier(InterpretedSymbols.maxF) +
        " ((x1 Real) (x2 Real)) Real\n  (ite (>= x1 x2) x1 x2))"),
  )

  /**
   * Convert KeYmaera X expression to SMT expression with negated formula form the result SMT expression is checked by
   * Z3 for satisfiability if SMT solver returns: unsatisfiable => original KeYmaera X formula `expr` is valid
  ...
  private[tools] def nameFromIdentifier(s: String): NamedSymbol = {
    ...
    else if (s.startsWith(DIFFSYMBOL_PREFIX)) {
      val (n, i) = toName(s.stripPrefix(DIFFSYMBOL_PREFIX)); DifferentialSymbol(Variable(n, i))
  ...
  protected def convertTerm(t: Term): String = {
    require(
      t.sort == Real || t.sort.isInstanceOf[Tuple],
      "SMT can only deal with reals or pairs, but not with sort " + t.sort,
    )
    t match {
      ...
      case Number(n) =>
        ...
        if (n.signum < 0) {
          ...
          assert(
            ...
            throw ConversionException("Term contains illegal numbers: " + t),
          )
